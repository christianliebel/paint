...
export class LineTool implements Tool {
  ...
  onPointerMove(x: number, y: number, { previewContext, canvas, lineWidth }: DrawingContext): void {
    if (canvas && previewContext) {
      this.drawLine(x, y, previewContext, previewContext, canvas, lineWidth);
    }
  }

  onPointerUp(x: number, y: number, { previewContext, context, canvas, lineWidth }: DrawingContext): void {

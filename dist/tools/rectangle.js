...
export class RectangleTool {
  ...
  onPointerUp(x, y, {canvas, context, lineWidth, fillStyle, previewContext}, color) {
  ...
  drawRectangle(x, y, targetContext, previewContext, fillStyle, lineWidth, canvas, color) {
    ...
    if (fillStyle.stroke) {
      targetContext.fillStyle = color.stroke.value;
      this.getPointsForLine(x1, y1, x2, y2, lineWidth).forEach(({x: x3, y: y3}) => {
        targetContext.fillRect(x3, y3, 1, 1);
      });
    }
  }

...
class App extends LitElement {
  ...
  constructor() {
    ...
    this.addEventListener('set-help-text', event => this.helpText = event.detail ?? defaultHelpText);
    this.addEventListener('coordinate', event => this.coordinateText = event.detail ? `${event.detail.x},${event.detail.y}` : '');
    this.addEventListener('area', event => this.areaText = event.detail ? `${event.detail.width}x${event.detail.height}` : '');
    this.addEventListener('drawing-context-changed', event => this.drawingContext = event.detail);
